...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    result = [0, 0]
    for i, v in enumerate(user_input_number):
        for j, w in enumerate(random_number):
            if v == w:
                if i == j:
                    result[0] +=1
                else:
...
def main():
    ...
    while True:
        ...
        if is_validated_number(user_input) == True:
            ...
            print(f"Strikes : {strike_ball[0]} , Balls : {strike_ball[1]}")
            if strike_ball == [3, 0]:
                while True:
                    Yes_or_No = input("You win, one more(Y/N) ?")
                    if is_yes(Yes_or_No) == True:
                        stop = False
                        random_number = str(get_not_duplicated_three_digit_number())
                        print("Random Number is : ", random_number)
                        break
                    elif is_no(Yes_or_No) == True:
                        stop = True
                        break
                    elif Yes_or_No == '0':
                        stop = True
